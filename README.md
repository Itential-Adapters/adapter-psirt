...
1. If you are running IAP 2019.1 or older, add the adapter properties for Psirt (created from Adapter Builder) to the `properties.json` file for your Itential build. You will need to change the credentials and possibly the host information below.
[Psirt sample properties](sampleProperties.json). If you are running IAP 2019.2 the adapter properties need to go into the database. You can review IAP documentation for how to do this.
...
The `healthCheck` call ensures that the adapter can communicate with Psirt. The actual call that is used is defined in the adapter properties.
```js
